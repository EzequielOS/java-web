...
public class ServletTest extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			/*
			 * Artista a1 = new Artista(null, "Leornardo Di Caprio", "US", new
			 * BigDecimal("2000000.00"), sdf.parse("20/03/2016")); try { as.inserir(a1);
			 * response.getWriter().append("Leonardo Di Caprio inserido!\n"); } catch
			 * (ServicoException e) { response.getWriter().append(e.getMessage()+"\n"); }
			 */

			///////////////////////////// Atualizando artista(1):
			/*
			...
			///////////////////////////// Atualizando artista(2):
			/*
			 * Artista a1 = as.buscar(1); try { a1.setNome("Cate Blanchett");
			 * as.atualizar(a1); response.getWriter().append("Artista atualizado!\n"); }
			 * catch (ServicoException e) { response.getWriter().append(e.getMessage() +
			 * "\n"); }
			 */

			///////////////////////////// Participacao nao repetida:
			/*
			 * Filme f1 = fs.buscar(2); Artista a1 = as.buscar(1); Participacao p = new
			 * Participacao(null, "Joaozinho", new BigDecimal("0.00"), f1, a1); try {
			 * ps.inserir(p); response.getWriter().append("Participacao inserida!\n"); }
			 * catch (ServicoException e) { response.getWriter().append(e.getMessage()); }
			 */

			///////////////////////////// Participacao repetida:
			/*
			 * Filme f1 = fs.buscar(2); Artista a1 = as.buscar(1); Participacao p = new
			 * Participacao(null, "Jack Dawson", new BigDecimal("0.00"), f1, a1); try {
			 * ps.inserir(p); response.getWriter().append("Participacao inserida!\n"); }
			 * catch (ServicoException e) { response.getWriter().append(e.getMessage()); }
			 */
